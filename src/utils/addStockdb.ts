...
// xlsx 파일 경로
const KOSPIFilePath = path.resolve(__dirname, "../../data/data_0204_20240617.xlsx");
const KOSDAQFilePath = path.resolve(__dirname, "../../data/data_5728_20240617.xlsx");
// Excel 파일을 읽고 데이터베이스에 삽입하는 함수
const insertExcelDataToDb = async () => {
  try {
    // Sequelize 모델 동기화
    await fs.access(KOSPIFilePath);
    console.log(`File found: ${KOSPIFilePath}`);
    // await fs.access(KOSPIFilePath);
    // console.log(`File found: ${KOSPIFilePath}`);
    // Read the Excel file
    const workbookKosdaq = XLSX.readFile(KOSPIFilePath);
    // const workbookKospi = XLSX.readFile(KOSPIFilePath);
    // await db.sequelize?.sync();
    // Excel 파일 읽기
    const sheetName = workbookKosdaq.SheetNames[0];
    const worksheet = XLSX.utils.sheet_to_json(workbookKosdaq.Sheets[sheetName]);
    for (const row of worksheet) {
      ...
      // 데이터베이스에 삽입
      try {
        const stfind = await Stock.findOne({
          where: { code: 종목코드 },
        });
        if (stfind) continue;
